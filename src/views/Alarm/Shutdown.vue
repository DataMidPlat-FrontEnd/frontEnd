  ...
  <div class="alarm-page">
    ...
    <el-card class="table-card">
      <div class="table-header">
        <h3>忘记关机报警统计</h3>
        <el-tag type="warning">报警类型: 忘记关机报警</el-tag>
      </div>
...
const handleQuery = async () => {
  ...
  try {
    ...
    if (response.code === '00000') {
      ...
      const filteredData = responseData.filter(item => item.type === 4)
...
const handleExport = () => {
  ...
  try {
    ...
    const exportData = allData.value.map(item => ({
      ...
      '报警ID': item.id || '-',
      '时间': item.time || '-',
      '报警类型': item.type === 4 ? '忘记关机报警' : '忘记关机报警',
      '详情': item.value || '-'
    }))
