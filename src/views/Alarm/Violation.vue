  ...
  <div class="alarm-page">
    ...
    <el-card class="table-card">
      <div class="table-header">
        <h3>违规行为报警统计</h3>
        <el-tag type="danger">报警类型: 违规行为报警</el-tag>
      </div>
      ...
      >
        ...
        <el-table-column prop="type" label="报警类型" min-width="140">
          <template #default="{ row }">
            <el-tag type="danger">
              <span v-if="row.type === 9">违规行为报警</span>
              <span v-else>-</span>
            </el-tag>
          </template>
        </el-table-column>
...
const handleQuery = async () => {
  ...
  try {
    ...
    if (response.code === '00000') {
      ...
      const filteredData = allData.filter(item => item.type === 9)
...
const handleExport = () => {
  ElMessageBox.confirm('确认导出当前查询结果吗？', '导出确认', {
    ...
    const exportData = tableData.value.map(item => ({
      ...
      '报警ID': item.id,
      '时间': item.time,
      '报警类型': item.type === 9 ? '违规行为报警' : '违规行为报警',
      '详情': item.value
    }))
    ...
    link.download = `违规行为报警统计_${new Date().toISOString().split('T')[0]}.csv`
    link.click()
