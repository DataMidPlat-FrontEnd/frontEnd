  ...
  <div class="alarm-page">
    ...
    <el-card class="table-card">
      <div class="table-header">
        <h3>维保到期报警统计</h3>
        <el-tag type="info">报警类型: 维保到期报警</el-tag>
      </div>
...
const handleQuery = async () => {
  ...
  try {
    ...
    if (response.code === '00000') {
      ...
      const filteredData = allData.filter(item => item.type === 6)
...
const handleExport = () => {
  ElMessageBox.confirm('确认导出当前查询结果吗？', '导出确认', {
    ...
    const exportData = tableData.value.map(item => ({
      ...
      '报警ID': item.id,
      '时间': item.time,
      '报警类型': item.type === 6 ? '维保到期报警' : '维保到期报警',
      '详情': item.value
    }))
    ...
    link.download = `维保到期报警统计_${new Date().toISOString().split('T')[0]}.csv`
    link.click()
