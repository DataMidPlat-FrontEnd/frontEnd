    ...
    <el-card class="page-header" v-loading="loading">
      ...
      <el-card class="table-card" v-if="total > 0">
        <el-table :data="tableData" border stripe height="540">
          <el-table-column prop="pi" label="课题组名称" min-width="220" />
          ...
          <el-table-column prop="tProject" label="项目数" min-width="100" :formatter="fmtInt" />
...
const fetchData = async () => {
  ...
  try {
    ...
    while (true) {
      ...
      const res = await getPiStatistics(params)
...
onMounted(() => {
  console.log('Pi.vue 组件挂载，准备加载数据...')
  fetchData()
})
</script>

<style lang="scss" scoped>
.usage-pi { padding: 16px; }
